...
import "../../styles/editor/SimplifyPage.css";
...
export default function SimplifyPage() {
  ...
  const API_SIMPLIFIED = "http://localhost:8080/api/editor/simplified";
  ...
  useEffect(() => {
    ...
    try {
      ...
      setEditorId(Number(payload.userId ?? payload.id ?? payload.sub ?? null));
    } catch (e) {
  ...
  // Load danh sách bài của Editor
  const refreshMine = useCallback(async () => {
    if (!editorId) return;

    const res = await axios.get(`${API_SIMPLIFIED}/mine/${editorId}`, {
      headers: authHeader,
      validateStatus: () => true,
    });
    setMySimplified(Array.isArray(res.data) ? res.data : []);
  }, [API_SIMPLIFIED, authHeader, editorId]);
  ...
  const handleApproveOne = async (item = currentMineItem) => {
    if (!editorId) {
      setMessage("⚠️ Không xác định được tài khoản editor.");
      return;
    }
  ...
  const handleApproveAll = async () => {
    ...
    try {
      const res = await axios.put(
        `${API_SIMPLIFIED}/approve-all/${editorId}`,
        null,
        { headers: authHeader }
      );
      await refreshMine();
  ...
  const handleHideFromUser = async (item = currentMineItem) => {
    ...
    try {
      ...
      setMessage("🗑️ Đã ẩn bản rút gọn khỏi user, editor vẫn có thể chỉnh sửa.");
    } catch {
  ...
  const handleHideAllFromUser = async () => {
    ...
    try {
      const res = await axios.put(
        `${API_SIMPLIFIED}/hide-all/${editorId}`,
        null,
        { headers: authHeader }
      );
      await refreshMine();
  ...
  return (
    ...
    >
              ...
              <div className="simplify-ai-panel">
                ...
                {isEditingDraft ? (
                  <div className="simplify-edit-area">
                    <textarea
                      className="simplify-input editor-mode"
      ...
      <section className="editor-workspace-panel simplify-history-panel">
        ...
        <section className="editor-workspace-panel simplify-management-panel">
